...
const Playlist = () => {
    ...
    const videosPage = pl?.videos ?? null;
    ...
    const handleDelete = async (row) => {
        ...
        const newItems = oldItems.filter((v) => v?.uri !== row.uri);
        ...
        if (!deletedWasCurrent) {
            const idx = currentUri
                ? newItems.findIndex((v) => v?.uri === currentUri)
                : 0;
        ...
        const oldIndex = oldItems.findIndex((v) => v?.uri === row.uri);
        const candidate =
            newItems[oldIndex] ?? newItems[oldIndex - 1] ?? newItems[0];
        const nextIndex = Math.max(
            0,
            newItems.findIndex((v) => v?.uri === candidate?.uri),
        );
    ...
    const queueUrisSig = queueItems.map(v => v?.uri).join('|');
